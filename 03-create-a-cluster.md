...
| 6 | 10.10.160.1 - 10.10.191.254 | 10.178.6.1 | 10.10.160.1 |
| 7 | 10.10.192.1 - 10.10.223.254 | 10.178.7.1 | 10.10.192.1 |
| 8 | 10.10.224.1 - 10.10.254.254 | 10.178.8.1 | 10.10.224.1 |
...
4. Launch controller (reference below for an NFS boot VM, could use hardware) NOTE: If booting once to find MAC address, subsequent boots will try to HD boot instead of network. To fix; `virsh destroy VM_NAME`, `virsh edit` (change `boot` from `hd` to `network`), `virsh start VM_NAME`
    ...
    virt-install --name=controller.${CLUSTERNAME} --boot uefi --ram=4096 --vcpus=2 --disk none --boot network --network bridge=${CLUSTER_NET},model=virtio --network bridge=${CLUSTER_NET},model=virtio --os-variant rocky9 --noautoconsole
    ```
5. Launch login node (with primary network interface (the one to PXE from) plugged into the Cluster network and secondary network interface plugged into the External network)
