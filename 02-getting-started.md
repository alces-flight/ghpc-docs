...
1. Download the Director & Domain appliances from [ghpc.openflighthpc.org](https://ghpc.openflighthpc.org) to `/data/ghpc-source/` on your VM host
2. Launch Director VM
    ...
    virt-install --name=${VM_HOSTNAME} --boot uefi --ram=2048 --vcpus=1 --import --disk path=${VM_IMAGE},format=qcow2 --disk path=${DATA_IMAGE},format=qcow2 --os-variant=rhel9.4 --network bridge=${NET_ETH0},model=virtio --network bridge=${NET_ETH1},model=virtio --graphics vnc,listen=0.0.0.0 --noautoconsole
    ```
